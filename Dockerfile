...
# 2. Instala librerías Python para scraping y MongoDB
...
# 3. Copia archivos de inicio (Asegúrate de que existan en tu carpeta)
COPY start-vnc.sh /usr/local/bin/start-vnc.sh
COPY supervisord.conf /etc/supervisor/conf.d/supervisord.conf

# Convierte saltos de línea Windows a Linux (CRLF a LF) y da permisos
...
# Inicia supervisord para manejar Jypyter + VNC simultáneamente
